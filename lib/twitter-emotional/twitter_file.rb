module TwitterFile
  ...
  def TwitterFile.write_all_tweets_to_file(filename, tweets)
    File.open(filename, 'a') do |file|
      tweets.each { |tweet| file.puts(tweet) }
    end
  end

  def TwitterFile.write_tweet_to_file(filename, tweet)
    File.open(filename, 'a') { |file| file.puts(tweet) }
  end

  def TwitterFile.read_tweets_from_file(filename)
